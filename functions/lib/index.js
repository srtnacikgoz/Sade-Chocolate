...
exports.calculateDailyStats = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        const productViewSessions = sessions.filter(s => ['product', 'cart', 'checkout', 'completed', 'abandoned'].includes(s.currentStage));
        const productViewUnique = new Set(productViewSessions.map(s => s.visitorId)).size;
        const cartSessions = sessions.filter(s => ['cart', 'checkout', 'completed', 'abandoned'].includes(s.currentStage));
        const cartAdditions = cartSessions.length;
        const cartUnique = new Set(cartSessions.map(s => s.visitorId)).size;
        const checkoutSessions = sessions.filter(s => ['checkout', 'completed'].includes(s.currentStage));
        ...
        const productCartCounts = {};
        sessions.forEach(s => {
            if (s.cartItemsDetail && Array.isArray(s.cartItemsDetail)) {
                s.cartItemsDetail.forEach((item) => {
        ...
        sessions.forEach(s => {
            ...
            const city = (_b = s.geo) === null || _b === void 0 ? void 0 : _b.city;
            if (city) {
        ...
        const locationLines = topCountries.map(([country, count]) => {
            if (country === 'Bilinmiyor')
                return `Bilinmiyor: ${count}`;
            // Bu ulkeye ait sehirler
            const countrySessions = sessions.filter(s => { var _a; return (((_a = s.geo) === null || _a === void 0 ? void 0 : _a.country) || 'Bilinmiyor') === country; });
            const countryCities = {};
            countrySessions.forEach(s => {
                var _a;
                const city = (_a = s.geo) === null || _a === void 0 ? void 0 : _a.city;
                if (city)
                    countryCities[city] = (countryCities[city] || 0) + 1;
            });
 ...
 */
exports.lookupOrder = functions.https.onCall(async (request) => {
    ...
    if (!orderNumber || !email) {
        ...
        throw new functions.https.HttpsError('invalid-argument', 'Siparis numarasi ve e-posta adresi gerekli');
    }
    ...
    try {
        ...
        const orderEmail = (((_a = data.customer) === null || _a === void 0 ? void 0 : _a.email) || '').toLowerCase();
        ...
        return {
            ...
            order: {
                ...
                customer: {
                    name: ((_b = data.customer) === null || _b === void 0 ? void 0 : _b.name) || '',
                },
                ...
                payment: {
                    method: ((_c = data.payment) === null || _c === void 0 ? void 0 : _c.method) || '',
                    status: ((_d = data.payment) === null || _d === void 0 ? void 0 : _d.status) || '',
                    total: ((_e = data.payment) === null || _e === void 0 ? void 0 : _e.total) || 0,
                    subtotal: ((_f = data.payment) === null || _f === void 0 ? void 0 : _f.subtotal) || 0,
                    shipping: ((_g = data.payment) === null || _g === void 0 ? void 0 : _g.shipping) || 0,
                },
