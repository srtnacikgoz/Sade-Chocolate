    ...
    return (
      ...
      >
        <div className="relative aspect-[4/5] bg-[#F9F9F9] dark:bg-gray-800 overflow-hidden">
          ...
          {/* Rozetler */}
          {product.badge && (
            <span className={`absolute top-0 left-0 text-white text-[10px] font-bold px-3 py-1 uppercase tracking-widest z-20 ${product.badge === 'New' ? 'bg-gold' : 'bg-gray-900 dark:bg-white dark:text-black'}`}>
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2 z-30">
            ...
            <button 
                ...
                className={`w-9 h-9 flex items-center justify-center bg-white/90 dark:bg-black/50 backdrop-blur-sm rounded-full transition-colors shadow-sm ${isFav ? 'text-red-500 hover:text-red-600' : 'text-gray-500 dark:text-gray-300 hover:text-red-500 dark:hover:text-red-400'}`}
            ...
            <button 
                ...
                className="w-9 h-9 flex items-center justify-center bg-white/90 dark:bg-black/50 backdrop-blur-sm rounded-full text-gray-500 dark:text-gray-300 hover:text-brown-900 dark:hover:text-white transition-colors shadow-sm"
        ...
        {/* Alt Bilgiler */}
        <div className="p-4 flex flex-col flex-grow relative z-20 bg-white dark:bg-dark-800">
          ...
          <button 
            onClick={(e) => handleAddToCart(e)}
            className="w-full mt-3 py-2 text-xs font-semibold uppercase tracking-wider text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-600 rounded hover:bg-gray-900 hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors"
          >
            {t('add_to_cart')}
          </button>
        </div>
      ...
      >
        <div className="relative aspect-[3/2] bg-[#F9F9F9] dark:bg-gray-800 overflow-hidden">
          ...
          <div className="absolute top-4 right-4 z-30 flex flex-col gap-2">
            <button 
                onClick={handleToggleFav}
                className={`w-10 h-10 flex items-center justify-center bg-white/90 dark:bg-black/50 backdrop-blur-sm rounded-full transition-colors shadow-sm ${isFav ? 'text-red-500 hover:text-red-600' : 'text-gray-500 dark:text-gray-300 hover:text-red-500 dark:hover:text-red-400'}`}
            >
              <span className="material-icons-outlined text-xl">{isFav ? 'favorite' : 'favorite_border'}</span>
            </button>
            <button 
                onClick={handleQuickView}
                className="w-10 h-10 flex items-center justify-center bg-white/90 dark:bg-black/50 backdrop-blur-sm rounded-full text-gray-500 dark:text-gray-300 hover:text-brown-900 dark:hover:text-white transition-colors shadow-sm"
            >
      ...
      >
        <div className="relative w-24 h-24 flex-shrink-0 bg-[#F9F9F9] dark:bg-gray-800 rounded-lg overflow-hidden mr-3">
          ...
          <div className="absolute top-1 right-1 flex flex-col gap-1 z-30">
             <button 
                onClick={handleToggleFav}
                className={`w-6 h-6 flex items-center justify-center bg-white/90 dark:bg-black/50 backdrop-blur-sm rounded-full transition-colors shadow-sm ${isFav ? 'text-red-500 hover:text-red-600' : 'text-gray-500 dark:text-gray-300 hover:text-red-500 dark:hover:text-red-400'}`}
             >
                <span className="material-icons-outlined text-xs">{isFav ? 'favorite' : 'favorite_border'}</span>
             </button>
             <button 
                onClick={handleQuickView}
                className="w-6 h-6 flex items-center justify-center bg-white/90 dark:bg-black/50 backdrop-blur-sm rounded-full text-gray-500 dark:text-gray-300 hover:text-brown-900 dark:hover:text-white transition-colors shadow-sm"
             >
