...
export const FocalPointPicker: React.FC<FocalPointPickerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/90 backdrop-blur-sm flex items-center justify-center p-8 animate-in fade-in duration-300">
      <div className="bg-white rounded-xl p-8 max-w-4xl w-full shadow-sm animate-in zoom-in-95 duration-500">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-mocha-900">Odak Noktası Seçin</h3>
              <p className="text-xs text-mocha-400 mt-0.5">Görselin hangi kısmına odaklanılsın?</p>
            </div>
          </div>
          {onClose && (
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-full hover:bg-cream-50 flex items-center justify-center transition-colors"
            >
        ...
        <div className="mb-6 space-y-3">
          <div
            ref={imageRef}
            onClick={handleClick}
            className="relative w-full h-96 bg-cream-100 rounded-2xl overflow-hidden cursor-crosshair group"
            style={{
          ...
          >
            ...
            >
              {/* Outer Ring */}
              <div className="absolute inset-0 border-4 border-white rounded-full shadow-sm animate-pulse"></div>
              {/* Inner Dot */}
              <div className="absolute inset-0 m-2 bg-gold rounded-full shadow-sm"></div>
              {/* Crosshair */}
          ...
          <div className="flex items-center justify-between bg-cream-50 rounded-xl p-4">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-mocha-400">X:</span>
                <span className="text-sm font-mono font-bold text-mocha-900">{focalPoint.x}%</span>
              </div>
              <div className="w-px h-4 bg-cream-200"></div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-mocha-400">Y:</span>
                <span className="text-sm font-mono font-bold text-mocha-900">{focalPoint.y}%</span>
              </div>
            </div>
            <button
              onClick={() => {
              ...
              className="text-xs font-bold text-mocha-400 hover:text-mocha-600 uppercase tracking-wider"
            >
              Sıfırla
            </button>
          </div>
        ...
        <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4">
          <div className="flex gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-blue-900 mb-1">İpucu</p>
