  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-mocha-900">Katalog Ayarları</h2>
          <p className="text-sm text-mocha-500 mt-1">Ürün kataloğunun görünümünü özelleştirin</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      <div className="bg-white rounded-xl border border-cream-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-mocha-900">Satır Başına Ürün Sayısı</h3>
            <p className="text-sm text-mocha-500">Desktop görünümde bir satırda kaç ürün gösterilsin</p>
          </div>
        </div>
                ...
                {Array.from({ length: option.value }).map((_, i) => (
                  <div
                    ...
                    className={`w-3 h-4 rounded-sm ${
                      settings.gridColumns === option.value ? 'bg-gold' : 'bg-cream-300'
                    }`}
                  />
                ))}
              </div>
              <div className="text-center">
                <span className={`font-semibold ${settings.gridColumns === option.value ? 'text-gold' : 'text-mocha-900'}`}>
                  {option.label}
                </span>
                <p className="text-xs text-mocha-500 mt-0.5">{option.description}</p>
              </div>
              {settings.gridColumns === option.value && (
      ...
      <div className="bg-white rounded-xl border border-cream-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-mocha-900">Varsayılan Görünüm</h3>
            <p className="text-sm text-mocha-500">Sayfa açıldığında hangi görünüm aktif olsun</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <button
            ...
            `}
          >
            <LayoutGrid size={24} className={settings.defaultViewMode === 'grid' ? 'text-gold' : 'text-mocha-400'} />
            <div className="text-left">
              <span className={`font-semibold ${settings.defaultViewMode === 'grid' ? 'text-gold' : 'text-mocha-900'}`}>
                Grid Görünüm
              </span>
              <p className="text-xs text-mocha-500">Kartlar yan yana</p>
            </div>
          </button>
          ...
          <button
            ...
            `}
          >
            <Rows3 size={24} className={settings.defaultViewMode === 'list' ? 'text-gold' : 'text-mocha-400'} />
            <div className="text-left">
              <span className={`font-semibold ${settings.defaultViewMode === 'list' ? 'text-gold' : 'text-mocha-900'}`}>
                Liste Görünüm
              </span>
              <p className="text-xs text-mocha-500">Kartlar alt alta</p>
            </div>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-cream-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-mocha-900">"Kendi Kutunu Oluştur" Kartı</h3>
            <p className="text-sm text-mocha-500">Özel kutu kartının katalogdaki konumu</p>
          </div>
        </div>
          ...
          {BOX_POSITION_OPTIONS.map((option) => {
            ...
            return (
              <button
                ...
                `}
              >
                <Icon size={20} className={settings.boxCardPosition === option.value ? 'text-gold' : 'text-mocha-400'} />
                <div className="text-left">
                  <span className={`font-semibold ${settings.boxCardPosition === option.value ? 'text-gold' : 'text-mocha-900'}`}>
                    {option.label}
                  </span>
                  <p className="text-xs text-mocha-500">{option.description}</p>
                </div>
              </button>
            );
          })}
      ...
      <div className="bg-white rounded-xl border border-cream-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-mocha-900">"Bonbon Koleksiyonu" Kartı</h3>
            <p className="text-sm text-mocha-500">Bonbon koleksiyon kartının katalogdaki konumu</p>
          </div>
        </div>
          ...
          {BONBON_POSITION_OPTIONS.map((option) => {
            ...
            return (
              <button
                ...
                `}
              >
                <Icon size={20} className={settings.bonbonCardPosition === option.value ? 'text-gold' : 'text-mocha-400'} />
                <div className="text-left">
                  <span className={`font-semibold ${settings.bonbonCardPosition === option.value ? 'text-gold' : 'text-mocha-900'}`}>
                    {option.label}
                  </span>
                  <p className="text-xs text-mocha-500">{option.description}</p>
                </div>
      ...
      <div className="bg-white rounded-xl border border-cream-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-mocha-900">Varsayılan Sıralama</h3>
            <p className="text-sm text-mocha-500">Katalog açıldığında ürünler nasıl sıralansın</p>
          </div>
        </div>
          ...
          {SORT_OPTIONS.map((option) => {
            ...
            return (
              <button
                ...
                `}
              >
                <Icon size={20} className={settings.defaultSortMode === option.value ? 'text-gold' : 'text-mocha-400'} />
                <div className="text-left">
                  <span className={`font-semibold ${settings.defaultSortMode === option.value ? 'text-gold' : 'text-mocha-900'}`}>
                    {option.label}
                  </span>
                  <p className="text-xs text-mocha-500">{option.description}</p>
                </div>
      ...
      <div className="bg-cream-50 rounded-xl border border-cream-200 p-6">
        ...
        >
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="aspect-[4/5] rounded-lg bg-cream-200 border border-cream-300" />
          ))}
        ...
        <p className="text-xs text-mocha-500 mt-3 text-center">
